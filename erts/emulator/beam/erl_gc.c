...
static void gather_binaries(ErtsWStack *wsp, Eterm *start, Eterm *stop) {
    Eterm* tp = start;
    while (tp < stop) {
        Eterm val = *tp++;
            ...
            switch (thing_subtag(val)) {
                case SUB_BITS_SUBTAG:
                {
                    ...
                    if (thing_subtag(underlying->thing_word) != HEAP_BITS_SUBTAG) {
                        for (UWord *bp = wsp->wstart; 1; bp += 1 + sizeof(ErtsDynamicWStack) / sizeof(UWord) + 1 ) {
                            if ((BinRef *)bp[0] == underlying) {
                                ErtsDynamicWStack *bws = (ErtsDynamicWStack *)(bp+1);
                                WSTACK_PUSH2(bws->ws, sb->start, sb->end);
                                break;
                            }
                            ERTS_ASSERT(bp < wsp->wsp);
                        }
                    }
                }
                ERTS_FALLTHROUGH();
...
erts_gather_binaries(ErtsHeapFactory *hfact, Process *rp) {
	...
	if (u.hdr->thing_word == HEADER_BIN_REF) {
            WSTACK_PUSH(binaries, (UWord)u.br);
            WSTACK_RESERVE(binaries, sizeof(ErtsDynamicWStack) / sizeof(UWord) + 1);
            WSTACK_INIT((ErtsDynamicWStack*)binaries.wsp, ERTS_ALC_T_ESTACK);
            binaries.wsp += sizeof(ErtsDynamicWStack) / sizeof(UWord) + 1;
        }
    }
	...
	gather_binaries(&binaries, heap, htop);
    }

    if (OLD_HEAP(rp))
        gather_binaries(&binaries, OLD_HEAP(rp), OLD_HTOP(rp) /*OLD_HEND(p)*/);

    gather_binaries(&binaries, HEAP_START(rp), HEAP_TOP(rp));
	...
	while (bp) {
            gather_binaries(&binaries,
                bp->mem, bp->mem + bp->used_size);
	    bp = bp->next;
	}
    }

    for (UWord *bp = binaries.wstart; bp < binaries.wsp; bp += 1 + sizeof(ErtsDynamicWStack) / sizeof(UWord) + 1 ) {
        BinRef *bin = (BinRef*)bp[0];
        ErtsDynamicWStack *ws = (ErtsDynamicWStack*)(bp+1);
        sz += 2 /* cons */ + 6 /* tuple (ptr, sz, refc, binary, subs) */;
        erts_bld_uword(NULL, &sz, (UWord) bin->val);
	erts_bld_uint(NULL, &sz, bin->val->orig_size);
        sz += ERL_REFC_BITS_SIZE;
        for (UWord *bits = ws->ws.wstart; bits < ws->ws.wsp; bits += 2) {
            sz += 2 /* cons */ + 3 /* tuple*/;
    ...
    for (UWord *bp = binaries.wstart; bp < binaries.wsp; bp += 1 + sizeof(ErtsDynamicWStack) / sizeof(UWord) + 1 ) {
        ...
        ErtsDynamicWStack *ws = (ErtsDynamicWStack*)(bp+1);
        Eterm bitslist = NIL;
        Eterm val = erts_bld_uword(&hp, NULL, (UWord) bin->val);
        Eterm orig_size = erts_bld_uint(&hp, NULL, bin->val->orig_size);
        Eterm binary;
        for (UWord *bits = ws->ws.wstart; bits < ws->ws.wsp; bits += 2) {
        ...
        erts_refc_inc(&bin->val->intern.refc, 1);
        Eterm bitstring = erts_wrap_refc_bitstring(
                &hfact->off_heap->first,
                &hfact->off_heap->overhead,
                &hp, bin->val, (byte*)bin->val->orig_bytes,
                0, bin->val->orig_size * 8);
        WSTACK_DESTROY(ws->ws);
        tuple = TUPLE5(hp, val,
                orig_size,
                /* We subtract the bump we did above when copying the binary */
                make_small(erts_refc_read(&bin->val->intern.refc, 1) - 1),
                bitstring, bitslist);
        hp += 6;
