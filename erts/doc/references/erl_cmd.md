...
- **`+JDdump true|false`{: #+JDdump }** - Enables or disables dumping of the
  machine code generated by the JIT. When enabled, each module loaded produces
  a file named after the module with the `.asm` suffix in the current working
  directory, containing an assembly listing of the emitted tier-1 native code.
  Defaults to false.

  This is a low-level debugging aid; the generated files can be large.

  Since: OTP 29.0

- **`+JT2enable true|false`{: #+JT2enable }** - Enables or disables the tier-2
  optimizing JIT. When enabled, every tier-2-eligible function is compiled to
  optimized native code and installed over its tier-1 code at module load time.
  Defaults to false.

  Since: OTP 29.0

- **`+JT2dump true|false|stderr|hir|lir|asm|stages|ra|all`{: #+JT2dump }** -
  Dumps the tier-2 JIT's intermediate representations and generated code for
  each function it compiles. This is independent of [`+JDdump`](#%2BJDdump),
  which dumps the tier-1 code. Defaults to false.

  This option can be combined multiple times to select an output sink together
  with one or more facets (for example `+JT2dump stderr +JT2dump ra`):

  - **`stderr`** - Route the dump to standard error.

  - **`true`** or **`file`** - Route the dump to a per-module file named after
    the module with the `.t2.asm` suffix, in the current working directory.

  - **`hir`** - The final high-level intermediate representation.

  - **`lir`** - The lowered, register-allocated low-level intermediate
    representation.

  - **`asm`** - A disassembly of the generated machine code.

  - **`stages`** - The intermediate high-level representation after each
    optimization stage (intrinsics, loop unrolling, and general optimization).

  - **`ra`** - The register-allocation result.

  - **`all`** - All of the facets above.

  - **`false`** or **`off`** - Disables all tier-2 dumping.

  Output sinks are mutually exclusive (the last one given wins); facets
  accumulate. A sink given with no facet selects the default `hir`, `lir` and
  `asm` view; a facet given with no sink defaults to the `stderr` sink.
