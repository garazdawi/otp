...
then
    if [ "${WSLcross}" != "true" ] ; then
        SPEC_FILE_PREFIX="$MAKE_TEST_REL_DIR/${SPEC_PREFIX}_${APPLICATION}.spec"
        SPEC_FILE="$MAKE_TEST_REL_DIR/$APPLICATION.spec"
    else
        SPEC_FILE_PREFIX=`w32_path.sh -m "$MAKE_TEST_REL_DIR/${SPEC_PREFIX}_${APPLICATION}.spec"`
        SPEC_FILE=`w32_path.sh -m "$MAKE_TEST_REL_DIR/$APPLICATION.spec"`
    fi
    if [ -f "$SPEC_FILE_PREFIX" ]; then
        SPEC_FILE="$SPEC_FILE_PREFIX"
   fi
    ARGS="-spec $SPEC_FILE"
fi
