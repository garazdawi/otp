...
| [`18_gha_test_plan.md`](18_gha_test_plan.md) | **GHA test plan for the T2 tier.** The T2 layer inventory (L1–L10) vs what the now-green standard matrix actually executes (mid-end only); the validated-but-unmerged safety net (`aarch64-ci`: ARM main.yaml leg + codegen-correctness fuzzer) and what NOT to port; four rollout waves (fuzzer → ARM leg → x86-cheap selftests/sweep/guard → tier-up CT + asan + blocking); vacuousness floors; acceptance criteria. |
| [`19_general_tier_path.md`](19_general_tier_path.md) | **The general-tier path.** Applying the evidence base to "make T2 generally usable": S0 = memo-17 census + map/term-heavy corpus + blocker-placement dimension (funded); staged S1–S6 (prototypes → gate rework → type-feedback profiler → census-ordered frontend classes → region compilation on the proven rung-2 machinery → default-on productization), each with gates and kill criteria; measured-vs-argued ledger; ~15–25% odds, S0+S1 buys the go/no-go. |
| [`20_census_results.md`](20_census_results.md) | **S0 census results — the numbers.** The `{t2_census, Bin}` instrument (frontend-only, real oracle, validated vs memo 14) + driver over three workloads. Union addressable coverage **59% json / 19.6% mapwl / 30.7% compiler** — clears 15% on all three, so the kill criterion does not fire. But the map "surprise" is muted (55% of map-heavy own-time is `maps` *library* calls, not opcodes), region compilation is priced at ≈ 0 out-of-loop, and the ceilings sit behind the twice-measured "eligible ≠ win." Verdict: advance to S1, scoped to the maps opcode prototype only, low prior. |
| [`22_where_time_goes_sampling.md`](22_where_time_goes_sampling.md) | **Sampling (perf) cross-check + the ⅓-addressable ceiling (2026-07-16).** Only ~⅓ of real time is in JIT-addressable Erlang code (rest is C runtime + GC); `call_time` over-weights small hot functions (`lists:foldl/3` 7.4%→<0.8%); the non-loop pool decomposes to body-recursive loops + wrappers + single map/set ops (~0 straight-line-GVN); forcing T2 on single-map-ops regresses 2.3×. Reinforces decided-against; names monomorphic map-shape specialization (S1) as the one supported lever. Includes the T2 perf-symbol hook (`$T2:M:F/A`) and the colima/perf recipe. |
| [`21_eligibility_exceptions_status.md`](21_eligibility_exceptions_status.md) | **Eligibility-widening mandate status (2026-07-16) — DONE.** The "bs position, maps, exceptions" coverage phase, all three legs landed + CI-green on the t2-correctness fuzzer. Current base-lib coverage **82.1% by function count** (16 577/20 181 install). Exceptions = Strategy-2 try (body in T2, unwind to T1's handler, handler dropped) + the **Y-frame error-exit fix** (a silent init_yregs SIGBUS that predated exceptions, now retired) + a fail-closed full unlock. Ranked remaining work (bare catch, call_fun, guard-BIF, try-of, bs, maps) and the recommendation: coverage is high, next value is strategic (S1 shape specialization), not more admission. |
