...
```
        ...
        └──────────────────────┬───────────────────────────────────┘
                               ...
                               ▼
        ┌──────────────────────────────────────────────────────────┐
        │  T2 manager (background compile thread)                  │
        │   - dequeues compile request {Mod,Fun,Arity}             │
        │   - reads SSA chunk + type feedback vector               │
        │   - runs T2 optimizer                                    │
        │   - installs the resulting blob                          │
        └──────────────────────┬───────────────────────────────────┘
                               │
                               ▼
        ┌──────────────────────────────────────────────────────────┐
...
| Compilation thread | **Dedicated background thread** | Like HotSpot's CompilerThread. Possibly mapped onto a dirty CPU scheduler if simpler; Phase-0 decision. |
...
The outer function's machine state at every BEAM instruction
boundary is **identical** to what T1 would have at the same boundary:
- Same X register values.
- Same Y register / stack frame contents.
- Same HTOP, FCALLS, c_p.

This is not just "compatible". It's exact-byte equality at every BEAM
instruction boundary in the *outer* function. Between boundaries
(within a single BEAM op's lowering) T2 has freedom to use scratch
CPU registers, untagged temporaries, etc.

### 6.2 What this buys us

1. **No framestates in the outer function.** Speculation failure at
   instruction K of T2's outer code = "jump to T1's compiled code
   for instruction K". The X/Y state is already correct.
2. **No register allocation pass for the outer function.** T1's
   register assignment IS T2's register assignment.
3. **T1 ↔ T2 interchangeable at any instruction boundary.** Tracing
   patches, breakpoint handlers, scheduler re-entry, GC resumption
   — all work transparently because T2 looks like T1 at every
   point they could fire.
4. **Trivial deopt for the outer function** (§9.1). The deopt stub
   is one instruction: `jmp t1_pc_for_this_instruction`.

### 6.3 What it constrains

T2's outer-function optimizations are limited to those that don't
change X/Y register state at instruction boundaries. The good news
is that most useful T2 optimizations don't:
- Removing a redundant guard (the next boundary's X/Y state is
  unchanged).
- Speculation-driven type narrowing (the speculate ops can be
  inserted at boundaries; subsequent BEAM ops compile to the same
  X/Y state regardless of how the speculative path is encoded).
- Constant folding within a boundary.
- CSE within a boundary.
- BIF open-coding (the resulting X register holds the same value).

Optimizations that *would* change X/Y state across boundaries — full
register reallocation, inter-instruction value reuse, deferred
flushing — are confined to **inlined regions**, where state has
already diverged.
...
### 7.5 Branch-frequency counters (v2)
...
- **Remote calls to monomorphic targets** where the target is
  pure or near-pure: inline if size ≤ threshold.
...
- **`call_fun` sites whose target is provably a known fun** via SSA
  ...
  inlined `lists:foldl/3`. No speculation.
...
### 11.2 No register allocation in the outer function

The outer function uses T1's X/Y register assignments verbatim
(§6). No allocation pass. Speculation-introduced temporaries live
in a small set of scratch CPU registers reserved for T2 use (e.g.
caller-save x9–x14 on aarch64).

### 11.3 Linear-scan allocator inside inlined regions

Inlined regions are the only place register allocation matters.
A simple linear-scan allocator over IR live intervals (LuaJIT
style; see `research/luajit.md`) maps inlined SSA values to
scratch CPU regs or stack slots. The allocator never touches
outer-function X/Y slots.
...
T2 compiles are queued; one runs at a time on the dedicated
background thread. If the queue is full, further requests get
...
- **T2 manager** background thread (does nothing yet).
...
- New profile slots for map-key tracking.
- Phase B/C lowering.
...
1. **State-preservation model breaks on a particular BEAM op.**
   Some BEAM ops have implicit state-dependent behaviour (e.g.
   binary match contexts, try-catch tags). If T2's lowering of
   these can't preserve T1's exact X/Y state at the next
   boundary, the model fails and we need framestates everywhere.
   **Mitigation**: Phase 1's identity transform exercises this
   directly; failure surfaces immediately.
...
- **Watchpoint granularity vs. cost.** Per-function planned;
  ...
  Phase 4.
- **SSA chunk format.** ETF vs custom compact encoding.
  Resolved in Phase 0.
- **Compilation thread vs dirty CPU scheduler.** Resolved in
  Phase 0 based on prototype experience.
- **Recompilation vs blacklisting on deopt-bouncy functions.**
  Probably exponential backoff with eventual blacklist; tune in
  Phase 4.
- **Profile-data lifetime across module reload.** Reset on
  reload v1; preservation across compatible reloads is v2.
- **Receive optimization** (`recv_mark`/`recv_remove` for
  ref-based receive shortcut). To be designed during Phase E
  (binary patterns through messaging).
- **Process dictionary.** Whether `get`/`put` need explicit
  side-effect modelling in the IR to prevent reordering.
  Investigate during Phase 5.
- **ETS / message-passing optimization.** Out of v1 scope but
  the long-tail win for real Erlang. Likely needs runtime
  cooperation beyond what T2 alone can provide.
...
- A separate IR family for register allocation (single linear-
  scan inside inlined regions is enough).
