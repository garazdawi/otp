begin
  ...
  EtsSave = lists:foldl(fun(Tab, {U, C, Cnt, Sk}) ->
      try case ets:info(Tab, compressed) of
          true -> {U, C, Cnt, Sk + 1};
          _ -> Ty = ets:info(Tab, type), Kp = ets:info(Tab, keypos), Os = ets:tab2list(Tab),
               Cl = ets:new(probetmp, [Ty, {keypos, Kp}, compressed, public]),
               ets:insert(Cl, Os), Um = ets:info(Tab, memory), Cm = ets:info(Cl, memory),
               ets:delete(Cl), {U + Um, C + Cm, Cnt + 1, Sk}
        end catch _:_ -> {U, C, Cnt, Sk + 1} end
    end, {0, 0, 0, 0}, ets:all()),
  ...
  [{otp, erlang:system_info(otp_release)}, {procs, length(Procs)},
   ...
   {ets_mb, erlang:memory(ets)/1048576}, {ets_unc_mb, EtsU*WS/1048576},
   {ets_comp_mb, EtsC*WS/1048576}, {ets_tables, EtsCnt},
   {ets_saved_mb, (EtsU-EtsC)*WS/1048576}, {shrink_gcall_saved_mb, (P0-P1)/1048576},
   {total_mb, T0/1048576}]
end.
