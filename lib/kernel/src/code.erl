...
-spec all_available() -> [{Module, Loaded}] when
...
all_available([], AllModules) ->
    ...
    AllAvailable = maps:fold(fun(File, Path, Acc) ->
                                     [{filename:rootname(File), filename:append(Path, File)} | Acc]
                             end, [], AllModules),
    OrderFun = fun F({A,_},{B,_}) ->
                       F(A,B);
                   F(A,B) when is_atom(A) ->
                       F(atom_to_list(A), B);
                   F(A,B) when is_atom(B) ->
                       F(A, atom_to_list(B));
                   F(A,B) ->
                       A =< B
               end,
