...
Finally there is `io_ansi:format/4` which does not return the string to be
printed, but instead sends it to the `t:io:device/0` that should handle it.
`io_ansi:format/4` works across nodes and will use the [terminfo] database
where the data is outputted to decide what to emit.
...
-export([format/1, format/2, format/3, format/4, enabled/0, enabled/1, scan/1]).
...
-type chardata() :: [unicode:unicode_binary() | code()].
...
-type foreground_color() :: black | blue | cyan | green | magenta | red | white | yellow |
                            ...
                            default_color | light_foreground_color().
...
-doc """
Scan the string for input terminal sequences.
""".
...
format_internal(Format, Data, Options) ->
    UseAnsi = proplists:get_value(enabled, Options, true),
    %% Only to be used by format/4
...
-spec format(io:device(), format(), [term()], options()) -> ok.
format(Device, Format, Data, Options) ->
    Ref = make_ref(),
    try
        lists:foldl(
              ...
              F(Ansi, ok) when is_atom(Ansi); is_tuple(Ansi) ->
                  case io:request(Device,{put_ansi, Options, Ansi}) of
                      {error, request} ->
                          ...
                          case proplists:get_value(ansi, Options, Ref) of
                              true ->
                            ...
                            Fun({TermInfoCap, AnsiFun}) when is_function(AnsiFun) ->
                                       try prim_tty:tigetstr(TermInfoCap) of
                                           {ok, TermInfoStr} ->
                                               ...
                                               TermInfoFun =
                                                   case Arity of
                                                       0 -> fun() -> prim_tty:tputs(TermInfoStr, []) end;
                                                       1 -> fun(A) -> prim_tty:tputs(TermInfoStr, [A]) end;
                                                       2 -> fun(A, B) -> prim_tty:tputs(TermInfoStr, [A, B]) end;
                                                       3 -> fun(A, B, C) -> prim_tty:tputs(TermInfoStr, [A, B, C]) end;
                                                       4 -> fun(A, B, C, D) -> prim_tty:tputs(TermInfoStr, [A, B, C, D]) end;
                                                       5 -> fun(A, B, C, D, E) -> prim_tty:tputs(TermInfoStr, [A, B , C, D, E]) end;
                                                       6 -> fun(A, B, C, D, E, F) -> prim_tty:tputs(TermInfoStr, [A, B , C, D, E, F]) end;
                                                       7 -> fun(A, B, C, D, E, F, G) -> prim_tty:tputs(TermInfoStr, [A, B , C, D, E, F, G]) end;
                                                       8 -> fun(A, B, C, D, E, F, G, H) -> prim_tty:tputs(TermInfoStr, [A, B , C, D, E, F, G, H]) end;
                                                       9 -> fun(A, B, C, D, E, F, G, H, I) -> prim_tty:tputs(TermInfoStr, [A, B , C, D, E, F, G, H, I]) end
                                                   end,
...
default_mappings() ->
    #{ cursor =>
           [{"cup", fun(Line, Column) -> ["\e[",Line,";",Column,"H"] end}],
       cursor_home => {"home", "\eH"},
       cursor_up =>
           [{"cuu1", "\e[A"},
