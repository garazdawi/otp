...
- **`{log, true | false}`** - Tells the I/O server that it should log each
  I/O request. Requests will be logged at `info` level with the following
  report:
  ...
  Not all I/O servers support this option. Use `io:getopts/1` to check if
  it is available.

  > #### Note {: .info }
  >
  > The I/O servers in Erlang/OTP will set the [logger domain](`logger_filters:domain/2`)
  > to `[otp, kernel, io, input | output]`. By default `m:logger` will not print
