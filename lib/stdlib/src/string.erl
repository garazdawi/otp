...
%% Exports used by unicode_util.erl
-export([gc/1, cp/1]).
...
equal_nocase(A0, B0) ->
    case {cp(unicode_util:casefold(A0)),
          cp(unicode_util:casefold(B0))} of
...
equal_norm(A0, B0, Norm) ->
    case {cp(unicode_util:Norm(A0)),
          cp(unicode_util:Norm(B0))} of
...
str_to_gcl_and_length(S0) ->
    gcl_and_length(gc(S0), [], 0).

gcl_and_length([C|Str], Acc, N) ->
    gcl_and_length(gc(Str), [C|Acc], N+1);
...
str_to_indexmap(S) ->
    [M|L] = str_to_map(gc(S), 0),
    {M,L}.

str_to_map([], L) -> [#{}|L];
str_to_map([G | Gs], I) ->
    [M|L] = str_to_map(gc(Gs), I+1),
...
-dialyzer({no_improper_lists, [cp/1, gc/1]}).

%% Inline gc/cp to avoid loading unicode_util module for ASCII strings
    ...
    case T1 of
        ...
        _ -> %% Keep the tail binary.
            case cp_no_bin(T1) of
                ...
                binary_found -> unicode_util:gc_1(T1, CP1);
                T4 -> unicode_util:gc_1(T4, CP1)
            end
    end;
gc(<<>>) -> [];
    ...
    if CP1 < 256, CP1 =/= $\r ->
           case Rest of
               <<CP2/utf8, _/binary>> when CP2 < 256 -> %% Ascii Fast path
                   [CP1|Rest];
               _ -> unicode_util:gc_1(Rest, CP1)
           end;
        CP1 =:= $\r ->
            case string:cp(Rest) of
                [$\n|R1] -> [[$\r,$\n]|R1];
                _ -> unicode_util:gc_1(Rest, CP1)
            end;
      true -> unicode_util:gc_1(Rest, CP1)
    end;
gc([CP|T]) when ?IS_CP(CP) -> unicode_util:gc_1(T,CP);
gc(Str) ->
    case cp(Str) of
        {error,_}=Error -> Error;
        CPs -> gc(CPs)
